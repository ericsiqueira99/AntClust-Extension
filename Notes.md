...
## Another idea for text:
[ ] semantic clustering (clsutering of words in the same category together like apple, banana, peach in a cluster and red, blue, etc in another)
[ ] test other similarity functions between strings, like distance of a string.
...
-[Applied to AntClust] Threshold Adjustment: Multiply template by a sigmoid of age, so that younger ants have a tempalte that accepts more ants. adjust the similarity threshold based on the age of the ants.
...
- [x] Labroche_carvalho (R1_reinforcement,R2,R3,R4,R5,R6,R7,R8,)
- [x] Labroche_carvalho_age_penalty (R1_reinforcement,R2,R3,R4_age,R5_age,R6,R7,R8,R9)
  
- [x] Labroche_dropout (R1,R2,R3,R4,R5,R6)(dropout)
- [x] Labroche_age_penalty_dropout (R1,R2,R3,R4_age,R5_age,R6)(dropout)
- [x] Labroche_carvalho_dropout (R1_reinforcement,R2,R3,R4,R5,R6,R7,R8, R9)(dropout)
- [x] Labroche_carvalho_age_penalty_dropout (R1_reinforcement,R2,R3,R4_age,R5_age,R6,R7,R8, R9)(dropout)
...
-  Fitnessfunction measures the mean ARI fro different tests, ranging from min and max number of clusters and min and max values per cluster.
...
- Random idea: ant colony is used for decision making i think, look into that and check a possible adaptation.
...
- Could encode the word/action with a sentimental analysis. such as 1 = good, -1 = bad. (words like= kill, hurt, steal, hug, love, help)
- Initalize template at random between 0.5 with gaussian.
- If word below template = bad, if above = good.
- Acceptance = both ants have same label.
- Somehow the same rules.
- Take majority label. 

**Ethical Ant**
- M becomes the estimator of how successful the ant is
- Is initialized with the label alread (1 = for good, 0 for bad) based on sentimental analysis value of word.
